...
class CoinbaseCredentials:
    ...
    @classmethod
    def from_vault(cls) -> "CoinbaseCredentials":
        """
        Load credentials from vault (not implemented).
        
        This is a placeholder for future implementation of vault integration.
        For now, it simply calls from_env to load credentials from environment variables.
        
        Returns:
            CoinbaseCredentials instance
        """
        auth: VaultEntry = get_auth("coinbase")
        return cls(
            name=auth.key_name,
...
def place_order(
    ...
    product_id = success_resp.get("product_id", coinbase_symbol)
    side = success_resp.get("side", action)

    parts = product_id.split("-", 1)
    asset = parts[0] if len(parts) == 2 else product_id
...
def get_order_status(
    ...
    if not order_id or not isinstance(order_id, str):
        raise ValueError(f"Invalid order_id: {order_id}. Must be a non-empty string")
    
    # Get API base URL
    if api_base_url is None:
        api_base_url = os.getenv("COINBASE_API_BASE_URL", "https://api.coinbase.com")
    
    # Get authenticator and build headers
    authenticator = get_coinbase_authenticator()
...
def get_current_price(
    ...
    """
    Get the current price for an asset pair from Coinbase.
    ...
    Returns:
        Price object containing current price and timestamp
    ...
    # Construct product ID in Coinbase format
    product_id = f"{asset.upper()}-{quote.upper()}"
    ...
    # Coinbase provides the last traded price in the price field
    current_price = float(product.get("price", 0))
    
    # Get timestamp - use the product's time field if available
    timestamp = product.get("time", "")
    
    logging.info(f"Current price for {product_id}: ${current_price} (timestamp: {timestamp})")

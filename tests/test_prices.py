"""
Unit tests for src/prices.py.

All Cosmos DB calls are mocked via src.repo.
"""
...
class TestSavePrice:
    def test_delegates_to_persist_price(self):
        price = make_price()
        with patch("src.prices.persist_price") as mock_persist:
...
class TestGetPreviousPrice:
    def test_returns_first_result_when_prices_exist(self):
        ...
        with patch("src.prices.get_prices", return_value=[expected]) as mock_get:
    ...
    def test_returns_none_when_no_prices(self):
        with patch("src.prices.get_prices", return_value=[]):
            result = prices_module.get_previous_price("BTC", "USD", "1H", "test")
            assert result is None

    def test_passes_correct_arguments_to_repo(self):
        with patch("src.prices.get_prices", return_value=[]) as mock_get:
